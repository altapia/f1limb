...
async function getApuestasFiles() {
    let jsonfilesPaths: Path[] = await glob('src/content/*/apuestas.json', { stat: true, withFileTypes: true })
...
function pintaClasificacion(listClasificacion:User[], ordenGanancia:boolean){
    ...
    listClasificacion.forEach( (u, i) => console.log(`${i+1} - ${u.nombre}: \t${u.ganancia}€ \t${u.puntos}pts.`)) 
}
...
async function escribeFicheroClasificacion(){
    ...
    const promise = writeFile('src/content/clasificacion.json', JSON.stringify(listClasificacion));
    return promise
}
...
async function calcular(json: string) {
    ...
    try {
        ...
        clasificacionGP.forEach(u => {
            ...
            if(user && user.puntos!== undefined && user.ganancia!== undefined && u.ganancia!== undefined && u.puntos!== undefined){
